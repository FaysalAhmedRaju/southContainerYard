        ...
        <div class="col-md-12" style="background-color: /*#f8f9f9 */ paleturquoise ; border-radius: 20px;">
            ...
            <div  class="col-md-4 col-md-offset-5">
                ...
                <form name="income_expense_form" id="income_expense_form" onsubmit="return(validate());"
                      action="<?php echo base_url('admin_controller/update_user_registration');?>"
                      method="POST">
                    ...
                    <fieldset>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="name" name="name" value="<?php echo $edata->name ?>"
                                   aria-describedby="emailHelp"  placeholder="Enter Name" >
                            <?php echo form_error('name');?>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Email</label>
                            <input type="email" class="form-control" id="email" name="email" value="<?php echo $edata->email ?>" aria-describedby="email" placeholder="Enter email">
                            <?php echo form_error('email');?>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Username</label>
                            <input type="text" class="form-control" id="username" name="username" value="<?php echo $edata->username ?>"   aria-describedby="emailHelp"  placeholder="Enter Username"  />
                            <?php echo form_error('username');?>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Password</label>
                            <input type="password" class="form-control" id="password" name="password" value="<?php echo $this->encrypt->decode($edata->password) ?>" placeholder="Enter Password" /> <!--required=""-->
                            <?php echo form_error('password');?>
                        </div>
                        <?php //echo form_error('head_id');?>
                        <input type="submit" value="Update" class="btn btn-primary" id="hheeee" />
...
<script>
    ...
    function validate()
    {
        if(document.income_expense_form.name.value == "")
        {
            ...
            return false;
        }else if(document.income_expense_form.email.value == ""){
            alert( "Email Required!" );
            document.income_expense_form.email.focus() ;
            return false;
        }else if(document.income_expense_form.username.value == ""){
            alert( "Username Required!" );
            document.income_expense_form.username.focus() ;
            return false;
        }else if(document.income_expense_form.password.value == ""){
            alert( "Password Required!" );
            document.income_expense_form.password.focus() ;
            return false;
        }
    }
